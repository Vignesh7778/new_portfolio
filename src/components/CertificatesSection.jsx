...
const certificates = [
  ...
  },
  {
    ...
    icon: ShieldCheck,
    color: "theme-primary",
    link: "https://academy.hackandfix.com/certificate-page/?user=27868&course=76269"
  },
  {
  ...
  },
  {
    ...
    icon: CheckCircle2,
    color: "blue-400",
    link: "https://www.linkedin.com/posts/vignesh-murali-dharan_completed-frontend-web-development-workshop-activity-7345445689998340097-wNVm?utm_source=share&utm_medium=member_desktop&rcm=ACoAAEz5wOoBVVDLAbdIPDaqOLdkDjJINfZ_WCQ"
  }
];
...
const colorThemes = {
  ...
  "blue-400": {
    ...
    shadowHover: "group-hover:shadow-[0_0_30px_rgba(96,165,250,0.12)]",
      ...
      <div className="container mx-auto px-6 lg:px-12 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {certificates.map((cert, idx) => {
            const theme = colorThemes[cert.color] || colorThemes["theme-primary"];
            ...
            return (
              <Tilt key={idx} options={{ max: 12, scale: 1.02, speed: 600 }} className="h-full">
                <CardComponent
                  ...
                  className={`bg-[rgba(10,12,18,0.95)] backdrop-blur-md border border-white/5 ${theme.border} ${theme.borderHover} ${theme.shadowHover} p-6 rounded-xl group relative overflow-hidden transition-all duration-300 cursor-pointer h-full flex flex-col justify-between block`}
                  {...cardProps}
                >
                  ...
                  {/* Laser Scanline */}
                  <div className={`absolute top-0 left-0 w-full h-[2px] ${theme.barColor} opacity-0 group-hover:opacity-75 group-hover:animate-[scan-pass_2s_linear_infinite] pointer-events-none z-10`}
                       style={{
                         boxShadow: `0 0 10px ${theme.hex}`,
                       }}
                  />

                  {/* Issuer Icon Overlay (Background) */}
                  <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-25 transition-all duration-300 group-hover:scale-110 pointer-events-none z-0">
                     <cert.icon size={80} className={`${theme.text}`} />
                  </div>
                  ...
                  <div className="relative z-10 flex-grow flex flex-col justify-between">
                    <div>
                      <div className="flex justify-between items-start mb-6">
                        ...
                        <span className="font-mono text-[10px] uppercase tracking-wider text-theme-muted bg-white/5 px-2.5 py-1 rounded-sm border border-white/5 flex items-center gap-1.5">
                          {cert.type} {cert.link && <span className="text-yellow-500 font-bold">↗</span>}
                        </span>
                      </div>
                      ...
                      <p className="text-xs text-theme-muted font-mono tracking-wide">
                        {cert.issuer}
                      </p>
                    </div>
                    ...
                    <div className="mt-8 pt-4 border-t border-white/5 flex items-center justify-between text-[9px] font-mono text-theme-muted">
                      <div>
                        <span className="opacity-45">HASH:</span> <span className="text-white/60 font-bold">{`0x${(idx * 314159).toString(16).toUpperCase().padEnd(6, 'F').slice(0, 6)}`}</span>
                      </div>
